...
class ConfigHandler(FL):
    ...
    def read_config(special_config=None, file_path="config/config.cfg"):
        ...
        if not os.path.exists(file_path):
            ...
            LOG(f"Dosya bulunamadı: {file_path}, varsayılan ayarlarla yeni dosya oluşturuldu.")
        ...
        for line in file:
            ...
            if line and not line.startswith("#"):  # Yorum satırlarını yok say
                ...
                if key in config:
                    ...
                    LOG(f"Özelleştirilmiş ayarlarda bilinmeyen anahtar '{key}' bulundu.", "error")
        ...
        if special_config:
            if special_config in config.keys():
                ...
                LOG(f"Özelleştirilmiş ayarlarda bilinmeyen anahtar: '{special_config}'", "error")
                return None
        return config
