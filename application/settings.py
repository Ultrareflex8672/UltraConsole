...
from application.updater import Updater as UP
...
class SettingsMenu(MS):
    ...
    def settings(self, **kwargs):
        ...
        if kwargs.get("selected_key") == 1:
            LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) UltraConsole ayarlarını görüntüledi.")
            SettingsMenu.ayarlari_goster(MS.read_config())
        elif kwargs.get("selected_key") == 2:
            if my_profil_data[3] == 0:
                LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) UltraConsole ayarlarını güncelleme menüsüne girdi.")
                SettingsMenu.ayar_degistir(MS.read_config())
            else:
                LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) kullanıcısının UltraConsole ayar güncelleme talebi yetki yetersizliği nedeni ile reddedildi.")
            ...
            LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) UltraConsole modül ayarları menüsüne girdi.")
            SettingsMenu.menu_islem(**kwargs)
        elif kwargs.get("selected_key") == 4:
            LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) kendi profil hesabı menüsüne girdi.")
            SettingsMenu.profil(**kwargs)
        elif kwargs.get("selected_key") == 5:
            if my_profil_data[3] == 0:
                LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) kullanıcı yönetim menüsüne girdi.")
                SettingsMenu.users(**kwargs)
            else:
                LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) kullanıcısının kullanıcı yönetim menüsü erişimi yetki yetersizliği nedeni ile reddedildi.")
                MS.create_frame("Erişim Yetkisi Hatası", "Bu menüye erişiminiz reddedildi. Yönetici yetkisi gerekli.")
        elif kwargs.get("selected_key") == 6:
            LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) UltraConsole hakkında menüsne girdi.")
            SettingsMenu.about_UC(**kwargs)

    def menu_system(root_, menu_data_, type_=0, module_path_=[], module_name_=None, class_name_=None, init_data_=None, func_name_=None, **kwargs):
        from application.menu_olusturucu import MenuSystem
        ms = MenuSystem(menu_data_, type_=0, module_path_=[], module_name_=None, class_name_=None, init_data_=None, func_name_=None, **kwargs)
        ms.show_menu(root_)
            ...
            if secim.isdigit() and 1 <= int(secim) <= len(ayarlar):
                ...
                if yeni_deger:
                    ...
                    if eminmisiniz.lower() == "e":
                        LOG(f"{secilen_anahtar} ayarı {ayarlar[secilen_anahtar]} değerinden {yeni_deger} değerine değiştirildi.")
    ...
    def search_value(data, target):
        if isinstance(data, dict):
            for value in data.values():
                if SettingsMenu.search_value(value, target):  # İç içe dict varsa tekrar ara
                    return True
        elif isinstance(data, str):  # Nihai (yaprak) value kontrolü
            return data == target
        return False
    ...
    def menu_islem(**kwargs):
        ...
        with open(config["menu_file"], "r", encoding="utf-8") as file:
            menu_data_all = json.load(file)  # JSON formatında oku
        ...
        for key, value in menu_data[root].items():
            if not isinstance(value, dict):  # Eğer değeri bir sözlük değilse (yani alt menüsü yoksa)
                no_submenu_values = no_submenu_values+[value]
        

        options_for_selection = ["Ekle", "Sil", "Değiştir", "Eklenmiş Modülleri Gör", "Geri Dön"]
        os.system('cls' if os.name == 'nt' else 'clear')  # Konsolu temizle
        ...
        if operation == 2 or operation == 3:
                ...
                if my_profil_data[3] == 0:
                    if no_submenu_keys[int(selection)-1] in menu_data[root]:
                        ...
                        if warn.lower() == "e":
                            ...
                            LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) {str(no_submenu_keys[int(selection)-1])} adlı modülü sildi.")
                    ...
                    MS.create_frame("Erişim Yetkisi Hatası", "Bu menüye erişiminiz reddedildi. Yönetici yetkisi gerekli.")
                    LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) kullanıcının {str(no_submenu_keys[int(selection)-1])} adlı modülü silme girişimi yetki yetersizliği nedeni ile reddedildi.")
                ...
                if my_profil_data[3] == 0:
                    if no_submenu_keys[int(selection)-1] in menu_data[root]:
                                ...
                                while True:
                                    ...
                                    if new_name not in menu_data[root]:
                                        ...
                                        if warn.lower() == "e":
                                            ...
                                            SettingsMenu.save_menu(menu_data)
                                            LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) {ckey} modülünün adını {new_name} olarak değiştirdi.")
                        ...
                        if int(opt) == 2:
                            ...
                            if warn.lower() == "e":
                                LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) {ckey} modülünün {menu_data[root][ckey]} adlı fonksiyonunu {new_fonk} olarak değiştirdi.")
                        ...
                        if int(opt) == 3:
                                while True:
                                    ...
                                    if new_name not in menu_data[root]:
                                        ...
                                        if warn.lower() == "e":
                                            LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) {ckey} modülünün adını {new_name} olarak ve {cvalue} fonksiyonunu {new_fonk} olarak değiştirdi.")
                                            menu_data[root][new_name] = menu_data[root][ckey]
                                            del menu_data[root][ckey]
                    ...
                    LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) kullanıcısının {ckey} modülünün adını {new_name} olarak ve {cvalue} fonksiyonunu {new_fonk} olarak değiştirme talebi yetki yetersizliği ile reddedildi.")
                    MS.create_frame("Erişim Yetkisi Hatası", "Bu menüye erişiminiz reddedildi. Yönetici yetkisi gerekli.")

        elif operation == 1:
            
            # if no_submenu_keys == []:
            #     MS.create_frame("Menüde Mevcut Olan Modüller", "Menüye Eklenmiş Mevcut Modül Bulunmuyor", "info")
            # else:
            #     MS.create_frame("Menüde Mevcut Olan Modüller", "|"+"|   |".join(f"{i+1}. {item}" for i, item in enumerate(no_submenu_keys))+"|", "info")
            my_profil_data = list(kwargs.get("user_data"))
            static_modul_path = config.get("module_path")

            MS.create_frame(f"Yeni Modül Ekle", f"Bir sonraki aşamada '{static_modul_path}' klasöründe bulunan eklenmeye hazır modüller listelenecektir. Modülünüzü listede göremiyorsanız dosyanızı '{static_modul_path}' klasörüne eklediğinizden emin olun!", "info")

            if os.path.isdir(static_modul_path):
                module_files = [f[:-3] for f in os.listdir(static_modul_path) if f.endswith(".py")]
            else:
                module_files = []
                MS.create_frame("Modül Hatası", static_modul_path+" adlı dizin bulunamadı! Lütfen uygulama ayarlarından 'module_path' yolunun doğru girildiğinden emin olun yada modüllerinizi "+static_modul_path+" klasörüne taşıyın!", "info")

            options = []
            for i in module_files:
                if SettingsMenu.search_value(menu_data_all, i) == False and i.startswith("_") == False:
                # if i not in no_submenu_values:
                    options.append(i)
                elif i not in no_submenu_values and i.startswith("_") == False:
                    options.append(i)
            options = options+["Geri Dön"]

            if options != ["Geri Dön"]:
                os.system('cls' if os.name == 'nt' else 'clear')  # Konsolu temizle
                selection = int(MS.create_frame("Yeni Eklenecek Modül Dosyası Seçimi", options, "menu"))
            else:
                MS.create_frame("Yeni Modül Bulunamadı", "'"+static_modul_path+"' dizininde uygulamaya eklenmeye hazır yeni bir modül bulunamadı. Modül eklemek için önce modül dosyanızı '"+static_modul_path+"' dizinine ekleyin.", "info")
                selection = None
            
            if selection:
                new_value = options[selection-1]
                while True:
                    new_key  = MS.create_frame("Modül Adı Belirle", "'"+new_value+"' modülü için menüde gösterilecek modül adını giriniz. (İptal için: 'X')", "")
                ...
                if new_value.lower() != "x":
                    ...
                    if warn.lower() == "e":
                        LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) {new_key} adlı modülü {new_value} fonksiyonu ile ekledi.")
            ...
            if no_submenu_keys == []:
                MS.create_frame("Menüde Mevcut Olan Modüller", "Menüye Eklenmiş Mevcut Modül Bulunmuyor", "info")
                LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) eklenmiş modüller listesini görüntüledi.")
            else:
                MS.create_frame("Menüde Mevcut Olan Modüller", "|"+"|   |".join(f"{i+1}. {item}" for i, item in enumerate(no_submenu_keys))+"|", "info")
                LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) eklenmiş modüller listesini görüntüledi.")
    ...
    def profil(**kwargs):
        ...
        if selection == 1:
            ...
            if are_u_sure.lower() == "e":
                LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} için yeni ad: {new_name}.")
                ...
                SettingsMenu.menu_system(root, menu_data[root], 0, [], None, None, None, None, **kwargs)
                # ms = MS(menu_data[root], 0, [], None, None, None, None, **kwargs)
                # ms.show_menu(root)
            else:
        ...
        if selection == 2:
            ...
            are_u_sure=MS.create_frame("Kaydet", "Değişikliği Kaydetmek İstediğinize Emin misiniz?", "(E/H)\n")
            if are_u_sure.lower() == "e":
                LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} için yeni soyad: {new_name}.")
                SQL_ = SQL(database_path)
                SQL_.sql_update_user(my_profil_data[0], my_profil_data[1], my_profil_data[2], my_profil_data[3], my_profil_data[4], my_profil_data[5], my_profil_data[6], my_profil_data[7])
                SQL_.conncls()
                MS.create_frame("Başarılı", "Değişiklik İşlemi Başarı ile Yapıldı", "info")
        ...
        if selection == 3:
            while True:
                ...
                if SQL_.sql_read_users(new_name) == False:
                    ...
                    are_u_sure=MS.create_frame("Kaydet", "Değişikliği Kaydetmek İstediğinize Emin misiniz?", "(E/H)\n")
                    if are_u_sure.lower() == "e":
                        LOG(f"{my_profil_data[0]} ID numaralı kullanıcı için yeni kullanıcı adı: {my_profil_data[1]}.")
                        SQL_ = SQL(database_path)
                        SQL_.sql_update_user(my_profil_data[0], my_profil_data[1], my_profil_data[2], my_profil_data[3], my_profil_data[4], my_profil_data[5], my_profil_data[6], my_profil_data[7])
                        SQL_.conncls()
                        MS.create_frame("Başarılı", "Değişiklik İşlemi Başarı ile Yapıldı", "info")
                ...
                if SettingsMenu.hash_password_md5(cur_pass) == my_profil_data[2]:
                    ...
                    if new_pass_valid == True:
                        ...
                        if new_pass2_valid == True:
                            are_u_sure=MS.create_frame("Kaydet", "Değişikliği Kaydetmek İstediğinize Emin misiniz?", "(E/H)\n")
                            if are_u_sure.lower() == "e":
                                LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} parolasını değiştirdi.")
        ...
        if selection == 5:
            while True:
                ...
                if mail_valid == True:
                    ...
                    are_u_sure=MS.create_frame("Kaydet", "Değişikliği Kaydetmek İstediğinize Emin misiniz?", "(E/H)\n")
                    if are_u_sure.lower() == "e":
                        LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} için yeni e-posta adresi: {new_mail}.")
                        SQL_ = SQL(database_path)
                        SQL_.sql_update_user(my_profil_data[0], my_profil_data[1], my_profil_data[2], my_profil_data[3], my_profil_data[4], my_profil_data[5], my_profil_data[6], my_profil_data[7])
                        SQL_.conncls()
                        MS.create_frame("Başarılı", "Değişiklik İşlemi Başarı ile Yapıldı", "info")
        ...
        if selection == 6:
            while True:
                ...
                if tel_valid == True:
                    ...
                    are_u_sure=MS.create_frame("Kaydet", "Değişikliği Kaydetmek İstediğinize Emin misiniz?", "(E/H)\n")
                    if are_u_sure.lower() == "e":
                        LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} için yeni telefon numarası: {new_tel}.")
                        SQL_ = SQL(database_path)
                        SQL_.sql_update_user(my_profil_data[0], my_profil_data[1], my_profil_data[2], my_profil_data[3], my_profil_data[4], my_profil_data[5], my_profil_data[6], my_profil_data[7])
                        SQL_.conncls()
                        MS.create_frame("Başarılı", "Değişiklik İşlemi Başarı ile Yapıldı", "info")
                                                        ...
                                                        if email_validation == True:
                                                            while True:
                                                                ...
                                                                if tel_validation == True:
                                                                    ...
                                                                    SQL_.conncls()
                                                                    if role == 0:
                                                                        LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) {username} adlı yeni yönetici oluşturdu.")
                                                                        MS.create_frame("Yönetici Oluşturuldu", username+" adlı yönetici başarı ile oluşturuldu.", "info")
                                                                    if role == 1:
                                                                        LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) {username} adlı yeni kullanıcı oluşturdu.")
            ...
            while True:
                ...
                if my_profil_data[0] != user_list_data2[selection-1][0] and selection > 0 <= len(user_list_data2):
                    ...
                    if are_u_sure.lower() == "e":
                        ...
                        LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) {user_list_data2[selection-1][1]} adlı kullanıcıyı sildi.")
                                                                        ...
                                                                        while True:
                                                                            ...
                                                                            if tel_validation == True or check_tel == False:
                                                                                ...
                                                                                if are_u_sure.lower() == "e":
                                                                                    ...
                                                                                    SQL_.conncls()
                                                                                if role == 0:
                                                                                    LOG(f"{my_profil_data[0]} ID numaralı {my_profil_data[1]} ({my_profil_data[4]} {my_profil_data[5]}) {username} adlı kullanıcının bilgilerini güncelledi.")
    ...
    def about_UC(**kwargs):
        ...
        if selection == 3:
            ...
            print("Güncelleştirmeler Kontrol Ediliyor... 25%")
            print("█████████")
            time.sleep(1)
            os.system('cls' if os.name == 'nt' else 'clear')  # Konsolu temizle

            print("Güncelleştirmeler Kontrol Ediliyor... 50%")
            print("█████████████████████")
            time.sleep(1)
            os.system('cls' if os.name == 'nt' else 'clear')  # Konsolu temizle

            print("Güncelleştirmeler Kontrol Ediliyor... 75%")
            print("████████████████████████████████")
            time.sleep(1)
            os.system('cls' if os.name == 'nt' else 'clear')  # Konsolu temizle
            
            print("Güncelleştirmeler Kontrol Ediliyor... 100%")
            print("██████████████████████████████████████████")
            time.sleep(1)
            os.system('cls' if os.name == 'nt' else 'clear')  # Konsolu temizle
            ...
            if new_version > current_version:
                ...
                do_u_want_update = MS.create_frame("🛈 Güncelleme Mevcut", "UltraConsole 'un V"+str_new_version+" versiyonu şuanda kullanılabilir. Şuanki sürümünüz: V"+str_current_version+"   Şimdi güncellemek ister misiniz?", "(E/H)")
                if do_u_want_update.lower() == "e":
                    LOG(f"Otomatik güncelleştirme başlatıldı.")
                    updater.update()
            else:
                MS.create_frame("✓ UltraConsole Güncel!", "UltraConsole 'un son sürümü olan V"+str_current_version+" kullanıyorsunuz", "info")
